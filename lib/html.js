                        ...
                        let i = Math.round(Math.random() * 100000000)
                            switch(attr.value) {
                                case "_blank":
                                    ...
                                    node.setAttribute("onclick", "window.parent.newTab(window.parent.parseUrl(document.getElementsByClassName(\"a" + i + "\")[0].getAttribute(\"href" + i + "\"),'decode'))")
                                break
                                case "_parent" || "_top":
                                    node.setAttribute(`c-${attr.name}`, `${attr.value}`)
                                    attr.value = "_self"
        ...
        return [
            ...
            { 
                ...
                childNodes: [
                    {
                        nodeName: '#text',
                        value: `window.$c = new Corrosion({ window, codec: '${this.ctx.config.codec || 'plain'}',  prefix: '${this.ctx.prefix}', ws: ${this.ctx.config.ws}, cookie: ${this.ctx.config.cookie}, }); $c.init(); document.currentScript.remove();`
                    },
                ], 
                attrs: [],
            }
