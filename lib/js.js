...
class JSRewriter {
    ...
    createHead(url) {
        ...
        if (!self.$c && self.importScripts) {
            ...
            self.$c = new c({ url: '${url}', codec: '${this.ctx.config.codec || 'plain'}', serviceWorker: true,  window: self, prefix: '${this.ctx.prefix || '/service/'}', ws: ${this.ctx.config.ws || true}, cookies: ${this.ctx.config.cookies || false}, title: '${this.ctx.config.title}', }); $c.init();
        };\n`;
    };
    iterate(ast, handler) {
